...
Each calculator is its own tool with a strictly typed schema. The full v0.1 → v1.0 roadmap is in [`docs/CALCULATOR_INVENTORY.md`](docs/CALCULATOR_INVENTORY.md) (1,203 entries cross-walked against MDCalc, NCBI MedCalc-Bench, Nobrega-Medtech/nobra_calculator, vitaldb/medcalc, with primary-derivation PMID on every row). The 36 (actually 44) high-leverage MedCalc-Bench calculators with pre-existing numeric ground truth are detailed in [`docs/MEDCALC_BENCH_36.md`](docs/MEDCALC_BENCH_36.md). Pediatric calculators get their own focused catalog at [`docs/PEDIATRIC_CALCULATORS.md`](docs/PEDIATRIC_CALCULATORS.md) (92 rows with society endorsement — AAP/AHA/PALS/NRP/WHO/CDC/ESPGHAN/ISPAD — because every cross-domain catalog structurally underrepresents peds). Drug-dosing calculators live in `@openclinicalai/drugs`, not here — see [`docs/DRUG_DOSING_CALCULATORS.md`](docs/DRUG_DOSING_CALCULATORS.md) for the 64-tool plan and the calc-vs-drugs boundary rule (any tool needing an RxCUI input or returning a drug-specific output goes to `drugs`).

**Complexity is a first-class field on every calculator** (`formula` | `lookup` | `tree` | `multi-step`). The `list_calculators` discovery tool can filter by complexity; `describe_calculator` surfaces it. The distinction drives testing strategy:

- **`formula`** — single arithmetic expression (e.g. Cockcroft-Gault). Tested by numeric tolerance against MedCalc-Bench fixtures.
- **`lookup`** — additive points by component (e.g. CHA₂DS₂-VASc). Same testing pattern as formula.
- **`tree`** — rule-cascade criteria producing a categorical classification (e.g. Berlin ARDS severity, Duke endocarditis criteria). Output `result` is a string, not a number; `rule_trace` records which criteria fired. Tested via **branch coverage** on the rule cascade — every criterion-met / not-met path that drives a different classification — rather than numeric tolerance. MedCalc-Bench is exclusively numeric and does not cover tree-class calcs; their fixtures must be authored from society guidelines or clinical-case examples in the primary literature.
- **`multi-step`** — composite of several atomic calculators with interpretive logic layered on top (e.g. `calc_kidney_workup`). Tested against the underlying atomic calcs plus the interpretive text.
...
**IP-sensitive calculators we cannot ship under Apache-2.0.** The inventory flags four that require separate licenses and therefore cannot ship in this package: MMSE (PAR Inc. copyright), MoCA (MoCA Clinic licensing), STS Cardiac Surgery Risk (closed coefficients), and BOADICEA/CanRisk (non-commercial). They are widely used; their absence is acknowledged in the README rather than silently omitted.
