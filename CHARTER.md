...
Calc is the simplest MCP from a depth-vs-breadth perspective — formulas are formulas, no graph to traverse. Two investment areas still matter:
...
2. **MedCalc-Bench validation in CI.** Every calculator must pass numeric validation against MedCalc-Bench. Block release on any calculator that fails. This is the scientific-rigor differentiator that lets the comparative eval credibly claim parity with MDCalc-quality calc surface, despite reimplementing formulas from primary literature rather than licensing MDCalc.
